...
import { StoreObjectInitial } from "@/store/storeStore";
import {
  RegisterObject,
  RegisterStoreObject,
  StoreUpdate,
...
const breadBrumb = [
  ...
  {
    itemName: "Thông tin cửa hàng",
    itemLink: "/store/infomation",
  },
];
const StoreInformationPage = () => {
  ...
  const handlePost = useMutation({
    mutationFn: (body: any) => postDataStore(body, "/store/change-info"),
    onSuccess: (data: StoreUpdate) => {
      toast.success("Thay đổi thông tin thành công!", {
        className: "p-4",
      });

      const item: StoreObjectInitial | null = currentStore
        ? currentStore
        : null;
      if (item) {
        item.storeName = data.name ? data.name : "";
        item.storeImage = data.image ? data.image : "";
  ...
  const handleSubmit = async (
    ...
    const body: StoreUpdate & {
      ...
      storeCode: currentStore?.storeCode.toString(),
      ...values,
    };
  ...
  const [initialValue, setInitialValue] = useState<
    StoreUpdate & { newImage?: File | null; newBannerImage?: File | null }
  >({
    name: "",
    phone: "",
    addressDetail: "",
  ...
  const convertAddress = (values: StoreUpdate) => {
    let result: string = "";
                ...
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="flex gap-x-2 shrink-0">
                    <ButtonForm
                      ...
                      type="submit"
                      loading={isLoading}
                      className="!w-24 bg-green-600 !rounded-md disabled:!bg-slate-700"
                    ></ButtonForm>
                  </div>
                </div>
                    ...
                    <div className="content-wrapper w-full mb-10">
                      <div className="mx-auto">
                        <div className="w-full bg-white sm:p-[30px] p-3">
                          <div className="grid grid-cols-[3fr_2fr] flex-col-reverse xl:space-x-11">
                            <div>
                              <div className="input-area">
                                ...
                                <div className="flex sm:flex-row flex-col space-y-5 sm:space-y-0 sm:space-x-5 mb-5">
                                  ...
                                  <SelectFormikForm
                                    ...
                                    itemValue={"name"}
                                    disabled={!fetchDistrict.data || isLoading}
                                    options={
                                      fetchDistrict.data
                                        ? fetchDistrict.data
                                        : []
                                ...
                                <div className="flex sm:flex-row flex-col space-y-5 sm:space-y-0 sm:space-x-5 mb-5">
                                  <SelectFormikForm
                                    ...
                                    disabled={!fetchWardCode.data || isLoading}
                                    name="wardCode"
                                ...
                                {errors.documentList &&
                                  touched.documentList && (
                                    <span className="text-xs text-red-500">
                                      Không để trống thông tin hình ảnh liên
                                      quan!
                                    </span>
                                  )}
                              </div> */}
                                ...
                                <div className="flex justify-center">
                                  <div className="w-full relative">
                                    <img
                                      ...
                                      alt=""
                                      className="w-full h-[300px] rounded-lg overflow-hidden object-cover object-top"
                                    />
                                    <input
                                      disabled={isLoading}
                                      onChange={(e) => {
