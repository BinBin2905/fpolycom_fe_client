...
const VoucherApply = ({
  ...
  return (
    ...
    >
      ...
      {save ? (
        <button className="cursor-pointer absolute top-1 right-1 py-1 px-2 text-xs flex items-center justify-center  bg-slate-400 text-white rounded-sm">
          Đã áp dụng
        </button>
      ) : (
        <button
          ...
          disabled={loading}
          className="cursor-pointer absolute top-1 right-1 py-1 px-2 text-xs flex items-center justify-center  bg-amber-400 text-white rounded-sm"
        >
          Áp dụng
        </button>
      )}
